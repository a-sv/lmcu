...
using lmcu_flags_t = std::enable_if_t<
  std::is_enum<_flags>::value && std::underlying_type_t<_flags>(_flags::lmcu_flags_object) == 0,
  _type
>;
...
template<bool _all, typename _flags, typename ..._bits>
constexpr inline lmcu_flags_t<_flags, bool> check(const _flags f, const _bits ...bits) noexcept
{
  return _all? all(f, bits...) : none(f, bits...);
}
