...
constexpr bool is_master_one_line_or_rxonly()
{
  constexpr auto m = module_t();
  return m.mode == mode::master && (m.direction == direction::one_line ||
                                    m.direction == direction::two_lines_rxonly);
}
...
uint16_t rx()
{
  ...
  if constexpr(is_master_one_line_or_rxonly<module_t>()) { enable<module_t, true>(); }
  ...
  if constexpr(is_master_one_line_or_rxonly<module_t>()) { enable<module_t, false>(); }
